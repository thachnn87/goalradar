 ...
 *
 * Table (run once via /api/newsletter/migrate or Vercel dashboard):
 *
 ...
 *
 * Required env var: POSTGRES_URL (set automatically by Vercel Postgres integration)
 */
...
export type UpsertResult =
  ...
  | { status: 'resent';    subscriber: Subscriber }   // unconfirmed, new token sent
  | { status: 'duplicate';                         }  // already confirmed — don't reveal
...
export async function upsertSubscriber(
  ...
  if (existing.rows.length > 0) {
    ...
    // Unconfirmed — refresh token so the link doesn't expire
...
export async function confirmByToken(token: string): Promise<ConfirmResult> {
  ...
  if (row.confirmed) {
    return { status: 'already_confirmed', subscriber: row };
  }
...
// ---------------------------------------------------------------------------
// Migration helper — called by /api/newsletter/migrate (one-time setup)
// ---------------------------------------------------------------------------
